...
HKIA endpoint
  ...
  → if 舉牌 pickup: whiteboard.py generates sign photo (fal.ai queue API)
    → send_photo to chat (fire-and-forget; failure → text fallback + /board retry)
...
- `ride_dispatch/whiteboard.py` — Whiteboard sign image generator. fal.ai queue API (GPT-Image-2 edit), async httpx, base image in `assets/`.
