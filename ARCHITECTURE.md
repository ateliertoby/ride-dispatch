...
Settlement (埋數)
  ...
  → a bar opens the batch sheet, a chip the credit sheet, a cell the day sheet;
    all three are reads, and a batch is created only by the bot's statement
    flow, so every batch traces back to the image it was read from
  → the header's 入數未對 count opens the work queue: open and partial credits,
    oldest first; a matched or archived credit is read off the calendar by date
  → batch sheet states what the bank has sent and what it still owes, one row
    per allocation and each of them a way into that credit's own sheet, and
    folds the order list behind a count; a short-paid batch takes its 未過數
    ticks on that same list
  → credit sheet states what arrived, how much of it a statement accounts for,
    and the batches it paid, with what each of those is still owed
...
HKIA parking endpoint (plate → inside? entry time, pvNr, paid)
  ...
  → two not-inside replies: close row (free/paid/gate), 已出閘 push, parking_fee written to the order
