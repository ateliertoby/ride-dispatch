...
8. Tap **$** to open 埋數: a month grid of what each day earned, coloured by whether the money is still to chase (amber), settled and waiting on the transfer (plain), or banked (green). Tap a day to read its legs and the batches they belong to; a batch the platform paid short shows what has arrived and what is still owed (已收 $2,950（08-24） · 未收 $510), with the legs it has not paid for tagged 未過數; 撤銷結算 unwinds a whole batch. Batches are not created here — a batch exists only because a statement screenshot was read (step 11), so every one of them traces back to the image it came from and on to the orders that image lists. The header also counts the bank ledger (入數 N 筆): tapping it lists every credit, which of them a statement accounts for, and which are still waiting for one. An order in a batch is locked against price and cancellation edits until it is unwound
