...
8. Tap **$** to open 埋數: an event calendar of the money. A day cell carries what is still to chase in amber, or the day's total in muted text once all of it is on a statement; a settlement batch is drawn as a bar over the service days it covers, labelled with the platform's figure and coloured by whether it waits on the transfer, was paid short, or is banked, with a day the platform held back drawn as a dashed segment pointing at it; a bank credit is a chip on its value date. A bar paid short splits its own fill at the fraction that has arrived, so how much is in and how much is owed reads off the calendar; a chip takes its colour from the batches it went into, so money already allocated to a batch that is still short stays amber instead of dimming away. Tapping a bar reads the money first: what the platform confirmed, what has arrived and what is still owed (已收 $2,950（08-24） · 差 $510), one row per credit with a 解除 beside it, then the order numbers folded behind a count, and for a batch paid short that same list is where the legs it has not paid for are ticked 未過數 until they add up to the shortfall. A short batch also lists the credits that could close it (等緊補數) and a collected one says which legs the make-up transfer was for (補收 8/27 on the leg, in the day view too), because which legs the platform held back is kept as history rather than cleared when the money lands. Tapping a chip reads one credit, the batches it paid, and the batches its unspent money could pay (可能對); tapping the cell still reads the day's legs and links to the batches they belong to; 撤銷結算 unwinds a whole batch. 對 puts a credit against a batch and 解除 takes it back off, both from the page, so a make-up payment no longer has to be matched in Telegram. Batches are not created here — a batch exists only because a statement screenshot was read (step 11), so every one of them traces back to the image it came from and on to the orders that image lists. The header counts the credits no statement accounts for yet (入數未對 N 筆) and tapping it opens that queue, oldest first, with a row whose match is unambiguous carrying that batch and its 對 button in place; a credit already matched is found on the calendar by its value date. An order in a batch is locked against price and cancellation edits until it is unwound
